...
1.5. **抖音单会话规则（重要）**：一个抖音账号同一时间只有一个有效网页会话——任何一次扫码登录都会签发新 sessionid 并使旧会话立即失效（已实测：SparkFlow 的聊天会话被本项目的 web login 扫码踢掉）。两项目（DouYinSparkFlow 聊天 + 本项目发布）**共享同一份会话 cookies**：`data/web_cookies.json` ↔ SparkFlow 的 `COOKIES_SAKURO_MAI` secret ↔ `~/.config/douyin_keepalive/cookies.json`。任何一方重新扫码后，必须把新 cookies 同步给另外两处，否则另一方必挂。日常运行（keepalive 心跳/发布回写）不会创建新会话，可放心共存。
...
- **发布策略（2026-08-17 调整）**：私人号低频——每天 3 轮（07:10/12:10/20:10）每轮 1 条；正文深度模式 600-800 字（抓取原文全文最多 8000 字喂 LLM）；文末自动加 `—— 本文由 AI 转录整理，仅供参考 ——`（config `llm.ai_footer` 可关）；LLM 空输出按 deferred 重试而非永久跳过
- **2026-08-17 风控解除验证**：手机端手动发布无阻碍；网页自动化在拟人化改造（系统 Chrome + 随机停顿 2-8s + 去 webdriver 指纹）后真实发布成功，无验证墙；短信验证码为软验证（代码会等待用户手动输入最多 180s，见 publish_sms_verify 截图）
- **B 方案（2026-08-17 启用）**：网页自动发布触发短信验证码（每发必验，观察期），改为「生成草稿 + 通知，用户抖音 APP 手动发布（APP 零验证）」——`main.py draft` 只出草稿并推送（Server酱/SMTP，见 .env.example），run.sh 已切到 draft 模式
- **话题标签**：抖音编辑器靠标签后空格触发蓝色话题转换，`_compose_text` 已在最后一个标签后补尾随空格（否则残留为正文）
- 发布成功判定：错误 toast 优先 → 成功 toast → 跳转管理页 15s 无错误；超时如实报失败
